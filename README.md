...
- Attachment is best effort and never fails the session. Servers nobody accepts are reported as a logged warning (prefix `pi-acp MCP bridge:`), whatever did attach stays attached, and the session proceeds either way. The optional per-server `_meta.directTools` hint is passed to the extension's direct-tool setting, and an extension that does not understand it keeps its own behavior — the upstream `pi-mcp-adapter`, for instance, exposes such servers behind its gateway instead of as native tools.
