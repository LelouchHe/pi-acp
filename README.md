...
- Attaching those definitions is best effort. Servers that no extension accepts are reported as a logged warning (prefix `pi-acp MCP bridge:`) and the session continues without them; a partially attached set keeps whatever did attach. The optional per-server `_meta.directTools` hint is passed to the extension's direct-tool setting, and an extension that does not understand it keeps its own behavior — the upstream adapter, for instance, exposes such servers behind its gateway instead of as native tools.
